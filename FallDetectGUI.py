...
input1 = 'VIDEO/Dataset-1/timber(1).mp4'
...
class MyThread(threading.Thread):
    def __init__(self, thread_id):
        ...
        self.State = '正常'
        self.state_msg = "对象{}'s Current State: ???, time: {}".format(self.thread_id, time.asctime(time.localtime()))
    ...
    def get_state(self, points, model):
        ...
        self.state_msg = "对象{}当前状态: {}".format(self.thread_id, self.State)
        # time.strftime('%H:%M:%S')
        time1 = time.strftime('%H:%M:%S')
        msg2 = "对象{}摔倒时间: {}".format(self.thread_id, time1)

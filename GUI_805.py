...
# 建立五个对象得到临时点数据
object_tem = np.ones((5, 17, 2))
object_tem[0] = np.ones((17, 2))
object_tem[1] = np.ones((17, 2))

object_tem[2] = np.ones((17, 2))
object_tem[3] = np.ones((17, 2))
object_tem[4] = np.ones((17, 2))
...
def get_similarity(point1, point2):
    """
    两个架构的相似程度（去除全0点的距离），距离越近表示二者的关系越有可能是一个人的前后帧架构
    :param point1: 架构1
    :param point2: 架构2
    :return: 架构1和架构2共17个点(去除（x，y）=（0，0）所在点)的距离之和
    """
    simi_score = 0
    for i in range(0, 17):
        x1 = point1[i][0]
        ...
        if not (x1 == 0 and y1 == 0) or (x2 == 0 and y2 == 0):
            simi_score += math.hypot(p3[0], p3[1])
        else:
            simi_score += 0
    return simi_score
...
def save_obj_frame(point, number):
    """
    与上一帧的架构对比，保存当前的point到相应的架构中
    :param point: 当前的point
    :param number: 当前poses的size
    :return: 使用point更新的架构的下标
    """
    global object_tem
    score = [0, 0]
    score[0] = get_similarity(object_tem[0], point)
    score[1] = get_similarity(object_tem[1], point)

    if 0 < number < 3:
        if get_similarity(object_tem[0], np.ones((17, 2))) == 0:
            object_tem[0] = point
            return 0
        elif get_similarity(object_tem[1], np.ones((17, 2))) == 0:
            object_tem[1] = point
            return 1
        else:
...
def get_lying_state(point):
    point = ycl(point).flatten()
    counter = 0
    for i in point:
        if 0 < i < 1000:
            counter += 1
...
def is_complementary(arr1, arr2):
    """
    判断poses里是否有互补pose
    :param arr1: (17,3)
    :param arr2: (17,3)
    ...
    for i in range(0, 17):
        (x1, y1) = (arr1[i][0], arr1[i][1])
        (x2, y2) = (arr2[i][0], arr2[i][1])
        if x1 == 0 and y1 == 0:
            complement_counter += 1
        elif x2 == 0 and y2 == 0:
            complement_counter += 1
        elif abs(x1 - x2) < 40 and abs(y1 - y2) < 40:
            complement_counter += 1
        else:
            complement_counter += 0
    if complement_counter > 15:
        return True
    else:
        return False


def true_pose(pose1, pose2):
    pose_zeros1 = np.sum(pose1 == 0)
    pose_zeros2 = np.sum(pose2 == 0)
    if pose_zeros1 <= pose_zeros2:
        return pose1
    else:
        return pose2


def correct_poses(poses):
    """
    合并互补的架构
    :param poses:poses
    :return:
    """
    if len(poses) == 3:
        new_poses = np.zeros((2, 17, 3))
        new_poses1 = np.zeros((1, 17, 3))
        if is_complementary(poses[0], poses[1]) and is_complementary(poses[0], poses[2]):
            new_poses1[0] = true_pose(poses[0], poses[1])
            new_poses1[0] = true_pose(poses[2], new_poses1[0])
            return new_poses1
        elif is_complementary(poses[0], poses[1]):
            new_poses[0] = true_pose(poses[0], poses[1])
            new_poses[1] = poses[2]
        elif is_complementary(poses[0], poses[2]):
            new_poses[0] = true_pose(poses[0], poses[2])
            new_poses[1] = poses[1]
        elif is_complementary(poses[1], poses[2]):
            new_poses[0] = true_pose(poses[1], poses[1])
            new_poses[1] = poses[0]
        else:
            return poses
        return new_poses
    elif len(poses) == 2:
        new_poses = np.zeros((1, 17, 3))
        if is_complementary(poses[0], poses[1]):
            new_poses[0] = true_pose(poses[0], poses[1])
            return new_poses
        else:
            return poses
...
def obj_state_circle(data_1, data_2, data_3):
    """
    :param data_1: 存入3帧的人数信息
    :param data_2: 当前pose的size，即检测到的人数
    :param data_3: app显示人数
    :return: 更新后3帧的人数信息 app应该显示的人数
    """
    for i in range(0, 2):
        data_1[i] = data_1[i + 1]
    data_1[2] = data_2
...
class MyThread(threading.Thread):
    ...
    def get_state(self, points, model):
        ...
        # if predict_tag == 0 and result_lstm[predict_tag] > 0.9:
        #     predict_tag = 0
        # else:
        #     predict_tag = 1
        # if result_lstm[0][0] > 0.9:
        #     predict_tag = 0
        # else:
        #     predict_tag = 1
...
def main():
    ...
    win.ui.show()
    obj_n = 0
    obj_3 = np.array([0, 0, 0])
    tag_n = 1
    tag_3 = np.array([1, 1, 1])
    while True:
        ...
        if results:
            ...
            if space_counter % (fps_extract + 1) == 0:
                # print(poses)
                # print(size_p)
                size_p = poses.size / 3 / 17
                if size_p < 1:
                    obj_m = 0
                    obj_3, obj_n = obj_state_circle(obj_3, obj_m, obj_n)
                    win.ui.CurrentStateText.clear()
                    obj_number = '当前有{}人'.format(obj_n)
                    ...
                    obj_m = 2
                    obj_3, obj_n = obj_state_circle(obj_3, obj_m, obj_n)
                saved_tag = np.array([1, 1])

                for pose in poses:
                    points_obj = get_points(pose, output_transform2)
                    obj = get_obj(points_obj)
                    msg, tag_timbering, msg2 = g_parse_list[obj].get_state(points_obj, model_1)
                    saved_tag[obj] = tag_timbering
                    # print(tag_timbering)
                    # global msg_temp
                    # if tag_timbering == 0 and msg_temp != msg2:
                    ...
                    #     if is_timber <= 0:
                    #         is_timber = 3
                    # elif tag_timbering == 0 and msg_temp == msg2:
                    #     win.ui.CurrentStateText.clear()
                    #     obj_number = '当前有{}人'.format(obj_n)
                    #     win.ui.CurrentStateText.append(obj_number)
                    #     win.ui.CurrentStateText.append(current_state[1])
                ...
                msg2 = "发现摔倒，时间: {}".format(time1)
                global msg_temp
                if saved_tag[0] == 0 or saved_tag[1] == 0:
                    tag_3, tag_n = obj_state_circle(tag_3, 0, tag_n)
                else:
                    tag_3, tag_n = obj_state_circle(tag_3, 1, tag_n)
                if tag_n == 0 and msg_temp != msg2:
                    # win.ui.FallTimeText.setText(msg)
                    win.ui.CurrentStateText.clear()
                    obj_number = '当前有{}人'.format(obj_n)
                    win.ui.CurrentStateText.append(obj_number)
                    ...
                    win.ui.FallTimeText.append(msg2)
                    msg_temp = msg2
                elif tag_n == 0 and msg_temp == msg2:
                    win.ui.CurrentStateText.clear()
                    obj_number = '当前有{}人'.format(obj_n)
                    win.ui.CurrentStateText.append(obj_number)
                    win.ui.CurrentStateText.append(current_state[1])
